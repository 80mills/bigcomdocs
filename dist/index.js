            ...
            try {
                ...
                switch (name) {
                    ...
                    case 'search_api_endpoints':
                        result = await this.apiTools.searchEndpoints(args.query, args.method, args.api_category);
                        break;
                    case 'get_api_spec':
                        result = await this.apiTools.getApiSpec(args.api_name);
                        break;
                    case 'get_endpoint_details':
                        result = await this.apiTools.getEndpointDetails(args.api_name, args.endpoint_path, args.method);
                        break;
                    case 'list_api_categories':
                        result = await this.apiTools.listCategories();
                        break;
                    case 'recommend_api_for_use_case':
                        result = await this.apiTools.recommendApiForUseCase(args.use_case, args.operation_type, args.scale);
                        break;
                    case 'build_http_request':
                        result = await this.apiTools.buildHttpRequest(args.api_name, args.endpoint_path, args.method, args.parameters, args.body, args.auth_type);
                        break;
                    // Documentation Tools
                    case 'search_documentation':
                        result = await this.documentationTools.searchDocumentation(args.query, args.section, args.limit);
                        break;
                    case 'get_documentation_section':
                        result = await this.documentationTools.getSection(args.section_path);
                        break;
                    case 'list_documentation_topics':
                        result = await this.documentationTools.listTopics();
                        break;
                    case 'get_code_examples':
                        result = await this.documentationTools.getCodeExamples(args.topic, args.language);
                        break;
                    // Schema Tools
                    case 'get_schema':
                        result = await this.schemaTools.getSchema(args.schema_name);
                        break;
                    case 'search_schemas':
                        result = await this.schemaTools.searchSchemas(args.query);
                        break;
                    // Health check
                    case 'health_check':
