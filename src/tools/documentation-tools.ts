...
export class DocumentationTools {
  ...
  async searchDocumentation(query: string, section?: string, limit: number = 10): Promise<string> {
    try {
      ...
      const output: string[] = [`Found ${results.length} documentation result(s) for '${query}':\n`];
  ...
  async getSection(sectionPath: string): Promise<string> {
    try {
  ...
  async listTopics(): Promise<string> {
    try {
      ...
      const output: string[] = [`# Available Documentation Topics (${topics.length} total)\n`];
  ...
  async getCodeExamples(topic: string, language?: string): Promise<string> {
    try {
      ...
      const output: string[] = [`# Code Examples for: ${topic}\n`];
