...
export class APITools {
  ...
  private dropshippingConfig!: DropshippingConfig;
  ...
  private initializeUseCasePatterns(): void {
    const patterns: Record<string, UseCasePattern> = {
      product_queries: {
  ...
  private initializeDropshippingConfig(): void {
  ...
  async searchEndpoints(query: string, method?: string, apiCategory?: string): Promise<string> {
    try {
      const cacheKey = `search_endpoints_${query}_${method || 'all'}_${apiCategory || 'all'}`;
      let cachedResult = this.config.getCache<string>(cacheKey);
  ...
  private formatEndpointResults(results: ParsedEndpoint[], query: string): string {
    const output: string[] = [`Found ${results.length} endpoint(s) matching '${query}':\n`];
  ...
  private groupResultsByAPI(results: ParsedEndpoint[]): Map<string, ParsedEndpoint[]> {
  ...
  async getApiSpec(apiName: string): Promise<string> {
    try {
  ...
  private formatApiSpecOutput(specData: any): string {
    const spec = specData.spec;
    const info = spec.info;
    
    const output: string[] = [`# ${info.title || specData.name} API Specification\n`];
  ...
  async recommendApiForUseCase(useCase: string, operationType?: string, scale?: string): Promise<string> {
    try {
      ...
      const matches: APIRecommendation[] = [];
  ...
  private async formatRecommendationOutput(bestMatch: APIRecommendation, useCase: string, useCaseLower: string): Promise<string> {
    const pattern = bestMatch.pattern_data;
    
    const output: string[] = [`# API Recommendation for: ${useCase}\n`];
    output.push(`**Recommended Pattern:** ${bestMatch.pattern}`);
    output.push(`**Description:** ${pattern.description}`);
    
    const isBulk = ['bulk', 'batch', 'mass', 'hundreds', 'thousands', 'multiple'].some(
      word => useCaseLower.includes(word)
    );
    ...
    for (const apiName of pattern.apis) {
      try {
        ...
        if (apiData) {
          ...
          const relevantEndpoints = apiData.endpoints.filter(endpoint => 
            pattern.methods.includes(endpoint.method) &&
  ...
  private formatHttpRequestOutput(
    ...
    const output: string[] = [`# HTTP Request for ${method.toUpperCase()} ${endpointPath}\n`];
  ...
  private formatCategoriesOutput(categories: any[]): string {
    const output: string[] = [`# BigCommerce API Categories (${categories.length} total)\n`];
  ...
  async getEndpointDetails(apiName: string, endpointPath: string, method: string): Promise<string> {
    try {
  ...
  private formatEndpointDetailsOutput(endpoint: ParsedEndpoint): string {
    const output: string[] = [`# ${endpoint.method.toUpperCase()} ${endpoint.path}\n`];
  ...
  async healthCheck(): Promise<{ status: string; parserHealth: any; cacheStats: any }> {
    try {
